/**
 * Admin Validation Rules Management Page
 *
 * Comprehensive interface for managing validation rules across fandoms.
 * Features filtering, searching, bulk operations, and rule management.
 *
...
export default function ValidationRulesList() {
  ...
  // User role will be handled by the unified layout
  ...
  const filteredRules = rules
    ...
    .sort((a: ValidationRule, b: ValidationRule) => {
  ...
  const handleSelectAll = () => {
    if (selectedRules.length === filteredRules.length) {
      ...
      setSelectedRules(filteredRules.map((rule: ValidationRule) => rule.id));
    }
  };

  const handleSelectRule = (ruleId: string) => {
    setSelectedRules((prev: string[]) =>
  ...
  const getUniqueFandoms = (): Fandom[] => {
    const fandomMap = new Map<string, Fandom>();
    rules.forEach((rule: ValidationRule) => {
      if (!fandomMap.has(rule.fandomId)) {
        fandomMap.set(rule.fandomId, {
          id: rule.fandomId,
          name: rule.fandomName,
        });
      }
    });
    return Array.from(fandomMap.values());
  };

  const getUniqueCategories = (): string[] => {
    return Array.from(new Set(rules.map((rule: ValidationRule) => rule.category)));
  };
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5">
          ...
          <div>
            ...
            >
              ...
              {getUniqueFandoms().map((fandom: Fandom) => (
